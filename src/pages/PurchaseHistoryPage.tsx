...
function CollapseHeader({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="flex w-full items-center justify-between rounded-2xl border border-white/10 bg-slate-900/80 px-4 py-3 text-left transition hover:border-emerald-500/30 md:px-5 md:py-4"
    >
...
function PurchaseMetaCard({ purchase }: { purchase: PurchaseLike }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-slate-900/60 px-4 py-3 md:px-5 md:py-4">
...
export default function PurchaseHistoryPage() {
  ...
  async function handleDeletePurchase(purchaseId: string) {
    ...
    try {
      ...
      alert("Unable to delete this visit right now.")
    } finally {
  ...
  async function handleSaveEditedPurchase() {
    ...
    try {
      ...
      alert("Unable to save visit changes right now.")
    } finally {
  ...
  return (
    <>
      <PageIntroPopup
        ...
        title="Visit History"
        description="Review your saved visits and open any entry to see product details and grams used. Older visits stay here for reference, even after they roll out of your active 30-day window."
      />

      <div className="space-y-4 md:space-y-6">
        <div className="px-1 md:px-2">
          <p className="text-[10px] font-semibold uppercase tracking-[0.28em] text-emerald-400 md:text-xs">
            BudBalance
          </p>
        ...
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4 md:gap-4">
          <div className="rounded-2xl border border-white/10 bg-slate-900/90 px-3 py-3 shadow-sm shadow-black/20 md:px-5 md:py-4">
            <p className="text-[11px] uppercase tracking-wide text-slate-400">
              Saved Visits
            </p>
          ...
          <div className="rounded-2xl border border-white/10 bg-slate-900/90 px-3 py-3 shadow-sm shadow-black/20 md:px-5 md:py-4">
            ...
            <p className="mt-1 text-xl font-semibold text-emerald-400 md:text-2xl">
              30 Days
            </p>
          </div>
        ...
        <div className="space-y-3 md:space-y-4">
          ...
          {openSection === "filters" && (
            <div className="rounded-3xl border border-white/10 bg-slate-900/90 p-4 shadow-lg shadow-black/20 md:p-5">
              <div className="mb-3 md:mb-4">
                <h2 className="text-sm font-semibold text-white md:text-base">Filters</h2>
                <p className="mt-1 text-xs text-slate-400 md:text-sm">
                  Filter by visit or dispensary.
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 md:gap-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full rounded-2xl border border-white/10 bg-slate-800/90 px-3 py-3 text-sm text-white outline-none focus:border-emerald-500 md:px-4 md:py-3.5 md:text-base"
                  >
          ...
          {openSection === "saved" && (
            <div className="rounded-3xl border border-white/10 bg-slate-900/90 p-4 shadow-lg shadow-black/20 md:p-5">
              {savedPurchases.length > 0 ? (
                ...
                <div className="rounded-2xl border border-white/10 bg-slate-900/60 px-4 py-4 text-sm text-slate-400 md:px-5 md:py-5 md:text-base">
                  No saved visits found for the current filters.
                </div>
              )}
            </div>
          )}
          ...
          {openSection === "older" && (
            <div className="rounded-3xl border border-white/10 bg-slate-900/90 p-4 shadow-lg shadow-black/20 md:p-5">
              {olderPurchases.length > 0 ? (
                ...
                <div className="rounded-2xl border border-white/10 bg-slate-900/60 px-4 py-4 text-sm text-slate-400 md:px-5 md:py-5 md:text-base">
                  No older visits found for the current filters.
                </div>
              )}
            </div>
          )}
          ...
          {openSection === "rules" && (
            <div className="rounded-3xl border border-white/10 bg-slate-900/90 p-4 shadow-lg shadow-black/20 md:p-5">
              <div className="grid gap-2 md:gap-3">
                <div className="rounded-2xl border border-white/10 bg-slate-900/60 px-3 py-3 md:px-4 md:py-4">
                  <p className="text-sm text-slate-200 md:text-base">
                    Full visit details are stored for reference and history.
                  </p>
                </div>

                <div className="rounded-2xl border border-white/10 bg-slate-900/60 px-3 py-3 md:px-4 md:py-4">
                  <p className="text-sm text-slate-200 md:text-base">
                    Visits marked as archived stay saved but do not reduce your current allotment.
                  </p>
                </div>

                <div className="rounded-2xl border border-white/10 bg-slate-900/60 px-3 py-3 md:px-4 md:py-4">
                  <p className="text-sm text-slate-200 md:text-base">
                    Visits that count toward your allotment roll out of the active window after 30 days.
                  </p>
                </div>
              </div>
            </div>
      ...
      {editingPurchase && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4">
          <div className="relative max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-3xl border border-white/10 bg-slate-950 p-5 text-white shadow-[0_0_40px_rgba(0,0,0,0.55)] md:max-w-3xl md:p-6">
            <button
              type="button"
              onClick={closeEditPurchase}
              className="absolute right-4 top-4 text-sm text-slate-400 transition hover:text-white"
              aria-label="Close edit visit"
            >
              ✕
            </button>
            ...
            <div className="mt-4 space-y-3 md:space-y-4">
              ...
              <div className="border-t border-white/10 pt-4 md:pt-5">
                <div className="mb-3 flex items-center justify-between">
                  <p className="text-sm font-semibold text-white md:text-base">Items</p>
                  <button
                    type="button"
                    onClick={addEditItem}
                    className="rounded-xl border border-emerald-500/20 bg-emerald-500/10 px-3 py-2 text-xs font-semibold text-emerald-300 transition hover:bg-emerald-500/15 md:px-4 md:py-2.5"
                  >
                    + Add Item
                  </button>
                </div>
                ...
                <div className="space-y-3 md:space-y-4">
                  {editItems.map((item, index) => (
                    <div
                      key={item.id}
                      className="rounded-2xl border border-white/10 bg-slate-900/70 px-3 py-3 md:px-4 md:py-4"
                    >
                      <div className="mb-3 flex items-center justify-between">
                        ...
                        {editItems.length > 1 && (
                          <button
                            ...
                            className="rounded-xl border border-rose-500/20 bg-rose-500/10 px-2.5 py-1.5 text-[11px] font-semibold text-rose-300 transition hover:bg-rose-500/15 md:px-3 md:py-2"
                          >
                            Remove
                          </button>
                        )}
              ...
              <div className="grid grid-cols-2 gap-3 pt-2 md:gap-4">
                <button
                  type="button"
                  onClick={closeEditPurchase}
                  disabled={isSavingEdit}
                  className="rounded-2xl border border-white/10 bg-slate-900 px-4 py-3 text-sm font-semibold text-white transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-60 md:py-3.5 md:text-base"
                >
                  Cancel
                </button>
                ...
                <button
                  ...
                  disabled={isSavingEdit}
                  className="rounded-2xl bg-emerald-600 px-4 py-3 text-sm font-semibold text-white transition hover:bg-emerald-500 disabled:cursor-not-allowed disabled:opacity-60 md:py-3.5 md:text-base"
                >
                  {isSavingEdit ? "Saving..." : "Save Visit"}
                </button>
              </div>
            </div>
          </div>
